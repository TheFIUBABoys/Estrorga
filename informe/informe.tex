...
%\begin{abstract}
%El trabajo consiste en el an\'alisis de las estructuras en distintas %organizaciones,
%comenzando por una entidad relevada por completo y analizadas sus %caracter\'isticas, seguido de casos de estudio sobre
%situaciones problem\'aticas conocidas en determinadas organizaciones.
%\end{abstract}
...
El trabajo consiste en el an\'alisis de las estructuras en distintas organizaciones, tratando de reconocer que módelo teórico se ajusta mejor a cada caso.\\
En primer lugar se detalla la selección de una empresa para relevar en base a determinados criterios de ponderación, los cuales determinaron la preferencia de la empresa elegido por ensima de las demás alternativas consideradas.\\
A continuación, se realiza un análisis completo de la entidad relevada reconociendo sus caracter\'isticas principales y proponiendo una mejora en su estructura. Para que esto fuera posible, se recurrió a una serie de reuniones con un representante de la empresa, el cual nos proporcionó la información requerida.\\
Por último, se estudian ciertos casos que revelan distintas problemáticas conocidas en determinadas organizaciones y se proponen posibles soluciones para mejorar sus situaciones.
...
Esto fue originándose por la incapacidad de responder a la exigencias del departamento de ventas con las planificaciones y la falta de adaptación mutua entre sectores.
			...
			Teniendo en cuenta está consideración y el momento actual que atraviesa la empresa(ya pasados varios años de su creación y volviendose cada vez más grande), la configuración estructural que mejor se adapta a la organización es una burocracias mecánica.
...
Por otro lado, la estandarización de productos y procesos, permitiría mejorar los tiempos y costos de producción. Para no empeorar la imagen entre clientes, podría mantenerse la amplia gama de productos no estandar pero a un precio de venta mucho mayor que los productos estandar y con tiempos de entrega mayores. Esto incentivaría la compra de productos de esta última categoría por parte de los clientes(cliente que se adapta a los productos de la empresa y no empresa que se adapta al cliente). Otra ventaja de esta solución es que se puede predecir mejor que productos se van a vender, por lo que tengo una menor incertidumbre y un stock mucho menos diversificado.
	...
	\begin{enumerate}
			...
			\end{itemize}
			Vale aclarar que tal cual lo vimos en clase, las comprar dentro de esta empresa no son parte de esta gerencia sino que se llevan a cabo dentro de otro departamento.-\\
			\item \textit{Mencionar que tipo de configuración estructural se adapta mejor a la organización de referencia}\\
			...
			La otra mejora planteada es la implementaci\'on de las normas ISO 9000. Para esto se debe promover un Jefe de Calidad que se encargue del control de los distintos procesos que serán certificados.\\ Adem\'as el gerente general debe agregar un nivel de supervisores en la jefatura de producción para lograr una mayor especialización en la  producción de los nuevos productos a desarrollar. Para esto deber\'an capacitar a los empleados que sean asignados a los nuevos productos.
...
Cada organizaci\'on es un caso particular, sin embargo es posible analizar sus rasgos m\'as caracter\'isticos para intentar catalogarla
dentro de alguna de las categor\'ias y calificaciones te\'oricas existentes. Esto es muy \'util ya que se puede reaccionar de alguna 
forma eficiente ya conocida si se conoce cual es el problema en t\'erminos conceptuales te\'oricos. \newline
Es importante entonces analizar cual es el tipo de estructura que se adapta mejor a cada organizaci\'on, para lo cual se necesita una
dosis de extenso relevamiento seguido de un an\'alisis detallado de la informaci\'on obtenida. \newline
Una organizaci\'on se transforma continuamente, por lo que se debe tambi\'en contemplar siempre la posibilidad de cambiar a  otro tipo 
de estructura mas adecuada, de lo contrario el crecimiento de la organizaci\'on puede ser perjudicado seriamente.
